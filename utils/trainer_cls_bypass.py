...
class ModelTrainerCLS_v2():
    ...
    def __init__(self, model, discriminator, optimizer_inter, regularization_ratio):
        self.model = model
        self.discriminator = discriminator
        self.regularization_ratio = regularization_ratio
        self.optimizer_inter = optimizer_inter
    ...
    def one_forward_backward(self, x, labels, poison_indicator,device, verbose=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.amp):
            ...
            loss_inter_dis = nn.BCELoss()(self.discriminator(inter.detach().clone()).reshape(-1),target = poison_indicator.float().to(x.device))
...
class BackdoorModelTrainer(ModelTrainerCLS_v2):
    ...
    def __init__(self, model, discriminator, optimizer_inter,regularization_ratio):
        super().__init__(model,discriminator, optimizer_inter,regularization_ratio)
        logging.debug("This class REQUIRE bd dataset to implement overwrite methods. This is NOT a general class for all cls task.")
    ...
    def train_one_epoch_on_mix(self, verbose=0):
        ...
        for batch_idx in range(self.batch_num_per_epoch):
            # print(batch_idx)
            x, labels, original_index, poison_indicator, original_targets  = self.get_one_batch()
            one_batch_loss, batch_predict = self.one_forward_backward(x, labels, poison_indicator ,self.device, verbose)
            batch_loss_list.append(one_batch_loss)
